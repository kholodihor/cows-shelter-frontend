...
const NewsModal = ({ isOpen, setShowModal }: NewsModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden  ">
      <div
        className={`absolute inset-0 overflow-hidden bg-black opacity-40 transition-opacity  duration-700  ${isOpen ? 'bg-opacity-40' : 'bg-opacity-0'
          } `}
        onClick={handleCloseModal}
      ></div>
      <div
        className={`right-50 top-50 absolute translate-x-0 transition-all duration-700 ${isOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        <div className="py-4">
          <div className="flex  items-center">
            ...
            <span className="text-sm font-normal">
              {language === 'uk' ? 'Опубліковано' : 'Posted '} 12.10.2024
            </span>
          </div>
        </div>
